...
A native macOS menu bar app that adds a system-wide "Speak with Speakable" service to the right-click context menu. Select any text, right-click, and have it spoken aloud using OpenAI's Text-to-Speech API.
...
```bash
git clone https://github.com/aspect-build/speakable.git
cd speakable
xcodegen generate
...
2. Find **"Speak with Speakable"** under **Text**
...
Speakable/
├── Sources/
│   ├── SpeakableApp.swift         # App entry point
...
│   └── Speakable.entitlements
└── SpeakableTests/                # Unit tests
```
...
# Build
xcodebuild -project Speakable.xcodeproj -scheme Speakable build | xcbeautify

# Test
xcodebuild -project Speakable.xcodeproj -scheme Speakable test | xcbeautify
```
